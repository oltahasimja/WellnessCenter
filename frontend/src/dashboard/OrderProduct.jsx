...
const OrderProduct = () => {
  ...
  const fetchOrderProducts = async () => {
    const response = await axios.get('http://localhost:5000/api/orderproduct');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (formData.id) {
      await axios.put(`http://localhost:5000/api/orderproduct/${formData.id}`, formData);
    } else {
      await axios.post('http://localhost:5000/api/orderproduct', formData);
    }
    fetchOrderProducts();
    setFormData({});
  };
  ...
  const handleDelete = async (id) => {
    await axios.delete(`http://localhost:5000/api/orderproduct/${id}`);
    fetchOrderProducts();
  };
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <div className="bg-white shadow-lg rounded-lg p-6 w-full max-w-3xl">
        <h1 className="text-3xl font-bold text-center mb-6 text-gray-700">OrderProduct Management</h1>
        ...
        <form onSubmit={handleSubmit} className="mb-6 space-y-4">
          ...
          <input 
            ...
            onChange={(e) => setFormData({ ...formData, mysqlId: e.target.value })}
            className="border p-3 rounded-md w-full focus:ring-2 focus:ring-blue-500 outline-none"
          />
          ...
          <table className="w-full border-collapse shadow-md rounded-md bg-white">
            <thead>
              <tr className="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                <th className="py-3 px-6 text-left">id</th><th className="py-3 px-6 text-left">mysqlId</th><th className="py-3 px-6 text-left">quantity</th><th className="py-3 px-6 text-left">pricePerUnit</th><th className="py-3 px-6 text-left">totalPrice</th>
              ...
              {orderproductList.length > 0 ? (
                orderproductList.map((item) => (
                  <tr key={item.mysqlId || item.id} className="border-b border-gray-200 hover:bg-gray-100">
                    <td className="py-3 px-6 text-left">{item.id}</td><td className="py-3 px-6 text-left">{item.mysqlId}</td><td className="py-3 px-6 text-left">{item.quantity}</td><td className="py-3 px-6 text-left">{item.pricePerUnit}</td><td className="py-3 px-6 text-left">{item.totalPrice}</td>
